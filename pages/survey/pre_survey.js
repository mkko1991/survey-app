...
export default function PreSurvey() {
    ...
    const emojis = [
        ...
        "/emoticon/ninth.png",
        "/emoticon/tenth.png",
        "/emoticon/eleventh.png",
        "/emoticon/twelfth.png",
        "/emoticon/thirteenth.png",
        "/emoticon/fourteenth.png",
        "/emoticon/fifteenth.png",
        "/emoticon/sixteenth.png"
    ];
    ...
    const onChangeEmoji = async (e) => {
        ...
        for (const emoji of copiedEmojis) {
            ...
            if (emoji.includes("eleventh")) {
                const fileList = await getFileList("/emoticon/eleventh");
                recommendedEmojis.push(...fileList);
            }

            if (emoji.includes("twelfth")) {
                const fileList = await getFileList("/emoticon/twelfth");
                recommendedEmojis.push(...fileList);
            }

            if (emoji.includes("thirteenth")) {
                const fileList = await getFileList("/emoticon/thirteenth");
                recommendedEmojis.push(...fileList);
            }

            if (emoji.includes("fourteenth")) {
                const fileList = await getFileList("/emoticon/fourteenth");
                recommendedEmojis.push(...fileList);
            }

            if (emoji.includes("fifteenth")) {
                const fileList = await getFileList("/emoticon/fifteenth");
                recommendedEmojis.push(...fileList);
            }

            if (emoji.includes("sixteenth")) {
                const fileList = await getFileList("/emoticon/sixteenth");
                recommendedEmojis.push(...fileList);
            }
        }
    ...
    return (
        <div className="pre_survey-wrapper">
            <div className="pre_survey-container">
                ...
                <div className="pre_survey-box">
                    <div className="pre_survey-question">
                        <p>4. 평소 모바일 메신저를 하루에 얼마나 자주 사용하시나요?</p>
                        <div className="pre_survey-options">
                            {[
                                '1시간 미만',
                                '1시간 ~ 2시간',
                    ...
                    <div className="pre_survey-question">
                        <p>5. 평소 모바일 메신저에서 이모티콘을 얼마나 자주 사용하시나요?</p>
                        <div className="pre_survey-options">
                            {[
                                '거의 사용하지 않는다',
                                '가끔 사용한다 (10건 중 1~2건)',
                    ...
                    <div className="pre_survey-question">
                        <p>6. 평소 가장 자주 사용하는 메신저 앱을 선택해 주세요. <span className="pre_required">(최대 2개)</span></p>
                        <div className="pre_survey-options">
                            {apps.map((opt) => {
                    ...
                    <div className="pre_survey-question">
                        <p>7. 선호하는 이모티콘 선택 <span className="pre_required">(필수★ 3개 이상 선택해 주세요)</span></p>
                ...
                <div className="pre_survey-box">
                    <button className={"pre_survey-next-button"} onClick={ableNextStep}>인터페이스 유형 실험 시작하기</button>
                </div>
            </div>
        </div>
    );
